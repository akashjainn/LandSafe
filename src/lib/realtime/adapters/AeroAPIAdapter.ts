...
export class AeroAPIAdapter {
  ...
  async poll() {
    ...
    try {
      ...
      let raws: AeroAPIRawFlight[] = [];
      if (json && typeof json === 'object') {
        const maybeFlights = (json as Record<string, unknown>)['flights'];
        if (Array.isArray(maybeFlights)) {
          raws = maybeFlights.filter(r => r && typeof r === 'object').map(r => r as AeroAPIRawFlight);
        }
      }
...
function mapAero(r: AeroAPIRawFlight): Flight {
  const origin = r.origin ? { iata: r.origin.code, lat: r.origin.lat, lon: r.origin.lon } : { lat: 0, lon: 0 };
  const destination = r.destination ? { iata: r.destination.code, lat: r.destination.lat, lon: r.destination.lon } : { lat: 0, lon: 0 };
  ...
  return { id: r.fa_flight_id || r.ident, callsign: r.ident, origin, destination, current, actuals: { atd: r.actual_off, ata: r.actual_on }, schedule: { std: r.scheduled_out, sta: r.scheduled_in }, diverted_to: r.diverted ? destination : null };
}
